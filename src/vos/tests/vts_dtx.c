...
vts_init_dte(struct dtx_entry *dte)
{
	...
	size = sizeof(struct dtx_actors) + sizeof(struct dtx_daos_target);

	D_ALLOC(actors, size);
	assert_non_null(actors);

	actors->da_tgt_cnt = 1;
	actors->da_grp_cnt = 1;
	actors->da_data_size = sizeof(struct dtx_daos_target);
	actors->da_tgts[0].ddt_id = 1;
