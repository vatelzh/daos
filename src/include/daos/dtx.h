 ...
 */
struct dtx_modification_group {
	/* How many touched shards in this group. */
	uint32_t			dmg_tgt_cnt;
	 ...
	 * the dmg_tgt_cnt.
	 *
	 * If all the shards 'dmg_index[1 - dmg_redundancy - 1]' are lost,
	 * then the group is regarded as unavailable.
	 */
	uint32_t			dmg_redundancy;

	/* The indexes for the shards in the dtx_daos_target array. For the
	 * leader group that is the first one in dtx_actors, 'dmg_index[0]'
	 * is for the leader, the other 'dmg_index[1 - dmg_redundancy - 1]'
	 * are the leader candidates for DTX recovery.
	 */
	uint32_t			dmg_index[0];
};
...
struct dtx_actors {
	 ...
	 * information inside 'da_data'.
	 */
	uint32_t			da_grp_cnt;

	/* sizeof(da_data). */
	uint32_t			da_data_size;

	/* The first 'sizeof(struct dtx_daos_target) * da_tgt_cnt' is the
	 * dtx_daos_target array. The subsequent are modification groups.
	 */
	union {
		char			da_data[0];
		struct dtx_daos_target	da_tgts[0];
	};
};
