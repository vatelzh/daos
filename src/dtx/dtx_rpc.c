...
{
	struct dtx_actors		*actors = dte->dte_actors;
	struct dtx_cf_rec_bundle	 dcrb;
	d_rank_t			 myrank;
	int				 rc = 0;
	int				 i;

	if (actors->da_tgt_cnt == 0)
		return -DER_INVAL;
	...
	for (i = 0; i < actors->da_tgt_cnt && rc >= 0; i++) {
		struct pool_target	*target;
		d_iov_t			 kiov;
		d_iov_t			 riov;

		rc = pool_map_find_target(pool->sp_map,
					  actors->da_tgts[i].ddt_id, &target);
		D_ASSERT(rc == 1);
