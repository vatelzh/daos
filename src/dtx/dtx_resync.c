...
dtx_iter_cb(uuid_t co_uuid, vos_iter_entry_t *ent, void *args)
{
	...
	D_ASSERT(ent->ie_dtx_actor_ds > 0);
	D_ASSERT(ent->ie_dtx_tgt_cnt > 0);

	size = sizeof(*dre) + sizeof(*actors) + ent->ie_dtx_actor_ds;
	D_ALLOC(dre, size);
	if (dre == NULL)
		return -DER_NOMEM;
	...
	dte = &dre->dre_dte;
	actors = (struct dtx_actors *)(dte + 1);

	actors->da_tgt_cnt = ent->ie_dtx_tgt_cnt;
	actors->da_grp_cnt = ent->ie_dtx_grp_cnt;
	actors->da_data_size = ent->ie_dtx_actor_ds;
	memcpy(actors->da_data, ent->ie_dtx_actors, ent->ie_dtx_actor_ds);
